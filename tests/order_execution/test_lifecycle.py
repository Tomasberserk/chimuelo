...
from tests.order_execution.conftest import make_order_orm
...
class TestSyncOrderStatus:
    ...
    def test_order_not_in_m3_raises(self, lifecycle_manager, mock_grid_state) -> None:
...
class TestSyncAllOpenOrders:
    def test_syncs_all_open_orders(self, lifecycle_manager, mock_client, mock_grid_state) -> None:
    ...
    def test_empty_open_orders_returns_empty_list(self, lifecycle_manager, mock_grid_state) -> None:
        mock_grid_state.get_open_orders.return_value = []
        results = lifecycle_manager.sync_all_open_orders("SOLUSDT")
        assert results == []
...
class TestHandleFillEvent:
    def test_marks_order_filled_in_m3(self, lifecycle_manager, mock_grid_state) -> None:
        mock_grid_state.get_order.return_value = make_order_orm(status="NEW")
    ...
    def test_returns_correct_order_status_update(self, lifecycle_manager, mock_grid_state) -> None:
        mock_grid_state.get_order.return_value = make_order_orm(status="NEW")
